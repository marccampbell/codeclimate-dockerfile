  ...
  if (line.trim().split(' ').length === 1) {
    ...
    var msgs = []
      ...
      run_checks.aptget_commands(args).forEach(function(aptget_command) {
        ...
        if (subcommand === 'install') {
          if (!run_checks.aptget_hasnorecommends(aptget_command)) {
            self.printIssue(fileName, lineNumStart, 'apt-get install without --no-recommends', 'apt-get install commands should include a --no-recommends flag');
          }
        }
      });
    } else if (cmd === 'cmd') {
      ...
      if (!checks.param_count_min(args, 2)) {
        self.printIssue(fileName, lineNumStart, 'ADD requires at least 2 arguments', 'ADD command found with less than 2 arguments.');
      }
      var hasWildcard = false;
      var sources = args.split(' ');
      ...
      sources.forEach(function(source) {
        ...
        if (!checks.is_dir_in_context(source)) {
          self.printIssue(fileName, lineNumStart, 'ADD source locations must be relative', 'ADD source locations must be local to the Docker build context.');
        }
      });
      ...
      if ((sources.length > 1) || hasWildcard) {
        if (args.slice(-1)[0] !== '/') {
          self.printIssue(fileName, lineNumStart, 'ADD dest must be dir because of source args', 'When ADD source includes wildcards or multiple locations, dest must be a directory');
        }
      }
    } else if (cmd === 'copy') {
